...
**Commit / PR and CI:** Pending draft PR creation and fresh-checkout CI. This entry will be updated with the commit, PR, and run result before handoff.
...
**Plan revisions and reasons:** Checked M1.7 only after malicious-string regressions, dependency-use search, clean installation, and full local checks passed. Expanded this dependency audit to compatible lockfile refreshes when `npm audit` found five fixable advisories; no declared version range or feature stack changed. Marked M1 complete after its stated exit gate passed locally; PR CI remains the independent fresh-checkout confirmation. Merged rendering/dependency workflow guidance into the existing `AGENTS.md`; no other task's acceptance criteria changed.
