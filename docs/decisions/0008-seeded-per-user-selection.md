...
The graph format still exposes a single edge set without build configuration or dataset identity. The report is not currently loaded by the browser or trainer, and a copied graph can be separated from its report. M3.7 must resolve that provenance and compatibility gap. M5 must evaluate any cap or alternative neighborhood policy on a split-first recommendation benchmark. Decision 0001's source-use and public-artifact holds remain unchanged.
