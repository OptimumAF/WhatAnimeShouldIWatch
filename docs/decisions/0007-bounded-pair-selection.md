...
| Streaming sample or heavy-hitter sketch | Can bound keys, but support and means are approximate after eviction/sampling | Would need a recorded seed, explicit error/coverage report, and validation; no streaming sketch is implemented |
...
Reversing user order and each rating row leaves the uncapped-input selection identical in tests. [Decision 0008](0008-seeded-per-user-selection.md) supersedes the first-N per-user cap with a seeded, order-independent subset and a separate build report. A support value under that cap counts co-raters in the processed subset. The budgets count pair visits and keys, not exact heap bytes; the synthetic benchmark measures process peak RSS but does not establish production capacity. M3.7 must put selection configuration, dataset identity, and truncation into a versioned graph artifact. M5 must evaluate ranking quality after splitting data. Decision 0001 continues to hold provider-derived collection, training, and publication.
