...
The existing `aggregateAnimePairs` sum/count correction makes its pair-preference mean order independent under the tested tolerance, but does not change its meaning. At the M3.3 baseline, the browser used positive edges to seed candidates and applied negative edges only after a candidate existed; `ml/train_graph_mf.py` took the absolute edge weight for attractive regularization. [Decision 0006](0006-signed-graph-evidence.md) subsequently changed v1 sign consumption without reinterpreting a signed similarity in place. The graph builder also computes user means from the full list before applying its first-N rating cap. [Sarwar et al. (2001)](https://www.ra.ethz.ch/CDstore/www10/papers/519/node14.html) describe adjusted cosine by subtracting user means; their [correlation definition](https://www.ra.ethz.ch/CDstore/www10/papers/519/node13.html) centers item vectors instead.
...
- [Decision 0006](0006-signed-graph-evidence.md) defines how M3.4 negative, positive, and neutral evidence affects visualization, candidate generation, and model regularization. Do not take the absolute value of a negative similarity and then use it as an attractive model constraint. The benchmark's co-disliked pair also shows why a positive item relationship is not by itself positive evidence about a new user's preference.
- [Decision 0007](0007-bounded-pair-selection.md) replaces first-encounter v1 output caps with exact, support-ranked selection inside separate pair-visit and candidate-key limits; input-budget exhaustion fails before graph export. M3.6 still must replace first-N per-user selection and measure runtime, memory, skipped data, and coverage. The prototype similarity scorer makes no scalability claim.
