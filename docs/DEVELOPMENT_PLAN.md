...
- [x] **M2.4 — Create provider-wide request scheduling.** Coordinate metadata, seasonal lists, imports, and crawler requests; honor runtime limits and `Retry-After`, use bounded exponential backoff with jitter, timeouts, cancellation, and retry budgets. Concurrency limits alone are not a complete rate policy.
- [x] **M2.5 — Make collection resumable and idempotent.** Track pages/checkpoints and per-user fetch outcomes. Validate a complete snapshot before atomically replacing a user's list; never delete previous ratings because a page timed out or a response was empty/private. Test changed scores and removed entries.
- [x] **M2.6 — Add schema migrations and provenance.** Record provider/source IDs, fetched time, genuine provider update times when available, import-run status, and normalized-field version. Do not label crawl timestamps as viewing/rating timestamps.
...
- [x] **M2.8 — Preserve imported history faithfully.** Retain status, progress, score scale, unscored seen items, and unmapped entries. Deduplicate by provider identity; show import-preview counts and merge/replace choices. Add file-size limits and safe parsers for each chosen export format.
...
- [x] **M3.3 — Decide and document edge semantics.** Distinguish pair preference, correlation, co-occurrence, and item similarity. Benchmark a corrected legacy statistic against support-shrunk item similarity; do not call the legacy mean of two centered scores a correlation.
- [x] **M3.4 — Test positive/negative/neutral behavior.** Cover opposite tastes, universally disliked items, uniformly high/low raters, no variance, one observation, and no overlap. Define how negative evidence is used separately by visualization, candidate generation, and model regularization.
- [x] **M3.5 — Replace first-encounter cap bias.** Evaluate minimum support, deterministic top-neighbor selection, and bounded candidate-generation strategies. Ensure an output-edge cap also has a separate input-work/memory budget; postprocessing top-K alone does not eliminate quadratic pair enumeration.
- [x] **M3.6 — Make approximation explicit and reproducible.** For any sampling or per-user cap, use a recorded seed and documented selection policy, not lowest-ID slices. Report skipped data, approximation level, runtime, peak memory, and resulting coverage.
- [x] **M3.7 — Version and migrate the graph format.** Export support, semantics, configuration, and dataset identity. Maintain a deliberate compatibility path or reject old incompatible versions clearly. Keep recommendation neighborhoods separate from graph visualization samples.
...
**M3 local exit evidence (2026-09-25):** The M3.1–M3.6 invented hand-computed, shuffled-input, seeded-selection, and fail-closed budget tests remain green. M3.7's compact/legacy v2 exports share identity and support, reconcile truncation counts, and generate a separately identified explorer sample. Browser contracts reject non-finite weights, broken references, unsupported or mixed versions, and mismatched explorer provenance. The full synthetic fixture, unit, Python, workflow, typecheck, build, and 35-case browser gate passed. This closes M3 locally; it does not authorize provider-derived regeneration, prove ranking quality, or clear the M5 evaluation gate.
...
- [x] **M4.1 — Introduce explicit preference semantics.** Separate seen/unrated, liked, disliked, and confidence/importance. Map provider scores deliberately rather than treating every positive numeric score as positive preference. Migrate existing watch weights conservatively and explain changed behavior.
- [x] **M4.2 — Centralize eligibility.** Apply seen status, user exclusions, include-only candidate sets, content preferences, catalog availability, and required filters consistently across graph, model, hybrid, and fallback paths. Exclusions win over inclusion. Define the meaning of the existing include control clearly.
- [x] **M4.3 — Make missing model data nonfatal.** Fall back from hybrid/model to an eligible graph or baseline ranking, show the actual engine in use, and preserve settings. Test absent, corrupt, incompatible, and partially mapped model artifacts.
- [x] **M4.4 — Implement a useful cold-start path.** With no preferences, offer transparent popularity/quality and genre-based exploration; with a few titles, use a supported content/neighbor baseline. Seasonal starter suggestions must not silently become the user's watched/liked history.
- [x] **M4.5 — Evaluate user-vector construction.** Compare the current item-embedding average with a regularized fold-in fit using observed preferences and item biases. Test the method deployed in the browser, not just stored training-user vectors. Retain the simplest method that satisfies quality and latency goals.
- [x] **M4.6 — Validate hybrid blending.** Compare existing min-max blending with a robust alternative such as rank fusion on validation data. Define missing-component behavior, equal scores, endpoint blend weights, negative scores, and deterministic tie-breaking. Do not present an uncalibrated score as a probability.
- [x] **M4.7 — Produce faithful explanations.** Include actual source titles, distinct evidence counts, normalization and blend effects, biases/prior contribution, and uncertainty. Test that displayed numeric contributions reconcile with the score where claimed; qualitative reasons must be labelled as such.
- [x] **M4.8 — Add diversity and franchise-aware eligibility.** Avoid filling the list with sequels or near-duplicate franchise entries; provide an option to allow them. Do not imply a title has no prerequisites when relationship data is unknown. Validate relevance/diversity tradeoffs rather than hardcoding genre quotas.
...
**M4 local exit evidence (2026-09-25):** M4.1 preference and import tests keep low/unscored/merely seen titles out of positive seeds; M4.2–M4.3 eligibility and fallback tests cover graph, model, hybrid, discovery, and missing optional assets; M4.7 numeric and qualitative explanation tests reconcile visible reasons with scoring. M4.8 applies a final-list selector after eligibility, with a reversible related-title option and explicit prerequisite uncertainty. Its predeclared invented top-three diversity/quality gate passed (two to three distinct families; NDCG@3 `1.000000` to `0.904977`). The complete local synthetic/mock gate passed 52 pipeline, 70 web unit, 10 ML, 5 workflow, and 57 browser tests. This closes M4's local exit gate; it does not establish ranking quality on permitted real data, provider rights, or a release gate.
...
- [x] **M5.1 — Split raw interactions first.** Define immutable train/validation/test IDs and deduplicate before splitting. Use true temporal splits only where reliable rating/viewing timestamps exist; otherwise use seeded per-user holdouts and disclose the limitation.
- [x] **M5.2 — Fit all learned preprocessing on training data only.** Compute user baselines, graph edges/support, popularity statistics, and any learned feature transforms from the training partition. Isolate validation/test labels from these steps. Define an allowed metadata snapshot separately.
- [x] **M5.3 — Add explicit leakage tests.** Modify held-out scores or hidden interactions and assert that training normalization, graph statistics, fitted parameters, and training hashes do not change. Fixed split IDs are essential to make this test meaningful.
...
**M5.1 local evidence (2026-09-25):** [Decision 0014](decisions/0014-raw-interaction-splits.md) fixes a raw-score-first, deduplicated, immutable three-way split contract. A checked-in invented manifest has 13 unique interactions after one identical duplicate and exactly 7/3/3 train/validation/test IDs; row order, scores, and already normalized fields cannot move membership. The loader rejects conflicts and malformed raw rows; the partition reader rejects a stale or tampered manifest. Present exports have no verified rating/viewing event times, so the reproducible path is a seeded per-user holdout with an explicit limitation. Synthetic verified event times exercise temporal ordering and tied-boundary rejection. The prior MF/Optuna/LightGCN scripts still use full-graph preprocessing; M5.2–M5.3 must wire train-only fitting and leakage tests before their metrics can count. M5's exit gate remains open.

**M5.2 local evidence (2026-09-25):** [Decision 0015](decisions/0015-train-only-preprocessing.md) defines a separate fixed anime metadata snapshot with no rating-derived fields. The new split-first MF route validates the raw manifest, passes only its training tuple to fitting, computes user means/centered rows and popularity from train, and calls the existing TypeScript pair aggregator locally with those rows only. The pinned fixture has 7 train rows, 4 candidate pairs with exact weights/support, and one positive regularization edge. Six focused tests cover exact values, a two-user support case, metadata separation, stale manifest rejection, held-out score independence of the fit, pair-budget failure, and deterministic seed-based model fitting. No validation/test metric is emitted. Existing compatibility MF/Optuna/LightGCN commands remain outside this leakage-safe route; their old metrics are still invalid M5 evidence. M5.3 must test fitted parameter and training-hash invariance under held-out score and hidden-interaction perturbations. M5's exit gate remains open.

**M5.3 local evidence (2026-09-25):** [Decision 0016](decisions/0016-fixed-split-leakage-regression.md) predeclares validation/test score edits and separate held-out interaction-ID replacements on the invented fixture. Fresh manifests are required, stale manifests fail, and every comparison asserts identical train IDs and raw train rows. Five focused tests exercise the same orchestrator as the CLI and compare user means, centered rows, popularity, signed pair weights/support/statistics, training arrays, positive graph edges, train/fit/model hashes, and every MF parameter exactly. A training-score positive control changes the hashes. Adding an interaction changes derived training membership and is rejected as an invalid fixed-split comparison. The full local synthetic/mock gate passed after one unrelated browser test timed out waiting for its page input; that test passed alone and the full 57-test browser rerun passed. The split-first path still emits no holdout metric; old MF/Optuna/LightGCN metrics remain invalid. M5.4–M5.9 and the M5 exit gate remain open.
...
| M3 | Complete: local exit gate passed | M3.1–M3.7 verified on synthetic inputs. [Decision 0009](decisions/0009-graph-v2-contract.md) separates the recommendation graph from the explorer sample and versions semantics, provenance, and truncation. The full fixture gate and [fresh PR CI](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36117665009) passed; provider-derived regeneration and ranking-quality evaluation remain separately gated. |
| M2 | In progress | M2.1–M2.6, M2.8, and M2.9 verified locally; M2.9 fixture PR [CI passed](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36091467488), M2.4's [fresh PR CI passed](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36100629660), and M2.5's [fresh PR CI passed](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36102144479). M2.6 adds transactional schema versions and restricted row/run provenance; [fresh PR CI passed](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36104137643) on implementation commit d050981. [Decision 0001](decisions/0001-provider-data-permissions.md) records provider holds; the [M2.2 audit](audits/public-artifacts-2026-09-24.md) confirms public per-user ratings and graph edges. [Decision 0002](decisions/0002-provider-workflow-gates.md) gates future jobs once merged. [Decision 0003](decisions/0003-provider-request-policy.md) records M2.4's rate policy; [decision 0004](decisions/0004-collection-provenance.md) records M2.6's timestamp and migration rules. M2.7 remains held for permitted source content. M2.8 local and mocked history, native-scale, preview, and file-parser checks passed; [PR #15 fresh CI passed](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36107616622) on implementation commit `6f79d0c`. Real provider export compatibility was not tested. The M2 exit gate remains open. |
...
| M4 | Complete: local exit gate passed | M4.1–M4.8 passed synthetic and mocked gates. M4.8's [decision 0013](decisions/0013-franchise-diversity.md) fixes the selector and invented relevance/diversity threshold; the full local gate passed 52 pipeline, 70 web unit, 10 ML, 5 workflow, and 57 browser cases. [M4.8's fresh PR CI](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36135023593) passed on implementation commit `e602c9a`; earlier M4.4–M4.7 fixture PR CI runs also passed. M5 still owns permitted split-first ranking evaluation, and provider rights/publication remain held. |
| M5 | In progress | M5.1 pinned raw-score split IDs and [decision 0014](decisions/0014-raw-interaction-splits.md); [fresh PR fixture CI](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36137012790) passed on `9442a25`. M5.2's [decision 0015](decisions/0015-train-only-preprocessing.md) and invented split-first MF route fit centering, popularity, pair weights/support, and model parameters from train rows with separate static metadata; [fresh PR CI](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36138780526) passed on `39565f6`. M5.3's [decision 0016](decisions/0016-fixed-split-leakage-regression.md) and five focused synthetic tests prove exact train/graph/MF invariance to fixed-membership held-out perturbations; [fresh PR CI](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36140131852) passed on `f926411`. M5.4–M5.9 and the milestone exit gate remain open. No holdout metric or production-data evaluation is claimed. |
| M6–M8, M10 | Pending | Follow dependencies and safety priorities; no claim of completion. |
...
| 2026-09-24 | M2.4 | Browser retries, crawler loops, and Python Jikan feature requests used independent delays; browser metadata could send three Jikan requests at once and seasonal reads had a separate path | Complete: TypeScript per-provider scheduler coordinates browser and crawler calls with rolling windows, response-header cooldowns, bounded retries/jitter/timeouts/budgets, and cancellation; Python Jikan builder has the corresponding process-local policy. [Decision 0003](decisions/0003-provider-request-policy.md) records limits and scope | Synthetic clock/transport tests passed for shared Jikan windows, AniList headers, `Retry-After`, timeout, budget, cancellation, and crawler pagination. Full local fixture/typecheck/15 pipeline + 29 web unit/7 ML + 5 workflow/build/28 browser suite passed; no provider call | Per-runtime limits cannot coordinate unrelated tabs or processes; any future parallel deployment needs a shared coordinator and renewed provider approval. Python `urlopen` cancellation is bounded by its 30-second timeout. M2.5 and the M2 exit gate remain open; permission and public-asset holds are unchanged |
| 2026-09-24 | M2.5 | Collectors inserted or updated scored rows only, left removed entries behind, and treated a page cap as a complete list | Complete: per-user SQLite status, page checkpoints, and staged entries; validate shape, IDs, scores, full preceding pages, and a nonempty short terminal page before one transaction replaces ratings and normalization. Both collector CLIs use this path | Mocked page and temporary SQLite tests passed for reopen/resume, timeouts, cancellation, empty/private and malformed responses, overlapping pages, score changes, removed and unscored entries, repeated import, and rollback on insertion failure. Full local fixture/typecheck/26 pipeline + 29 web unit/7 ML + 5 workflow/build/28 browser suite passed | A zero-length response cannot prove an empty list, so it never clears ratings; exact multiples of 300 cannot close on this route. Offset pagination has no source snapshot version, so concurrent source edits may escape detection. M2.6 owns formal migration/provenance; provider-use and public-asset holds remain; M2 exit gate stays open |
| 2026-09-24 | M2.6 | SQLite tables were created without a schema version; row source was implicit, dataset export always claimed the MAL route, and collection had no durable attempt history | Complete: transactional `user_version` 1–3 migration path, source/item mappings, per-row fetch/run and normalization versions, and terminal import-run outcomes. [Decision 0004](decisions/0004-collection-provenance.md) fixes timestamp meanings and legacy unknown handling | Synthetic migration tests preserve legacy ratings and M2.5 checkpoints, reject future/malformed/orphaned databases without partial writes, and verify source/fetch/run fields, resume provenance, superseded attempts, and nullable provider update times. Full local fixture/typecheck/36 pipeline + 29 web unit/7 ML + 5 workflow/build/28 browser suite passed | No verified provider update timestamp is supplied by the current site adapter; unrecognized fields and old rows stay null. Local fetch/commit times are not viewing/rating times. M2.7 still needs permitted source content, so M2.8 proceeds next with synthetic imports; permission/public-asset holds and M2 exit gate remain |

| 2026-09-25 | M2.8 | Existing file and username imports kept only positive, graph-mapped scores as weights, with no durable status, progress, source scale, unscored or unmatched history and no explicit merge/replace preview | Complete for the selected local text and bounded MAL-style XML formats and mocked username adapters: separate provider-keyed history in browser state/profiles, safe parsing and size limits, native AniList scale, preview counts and explicit merge/replace, with unscored seen items excluded from recommendations | Synthetic unit and browser tests cover duplicates, status/progress/scale, all five AniList scales, unknown catalog IDs, reload, profile restore, unchanged re-import, cancel/failure/quota preservation, XML DTD/malformed/oversize rejection, and no local-file provider request; full local fixture/typecheck/36 pipeline + 37 web unit/7 ML + 5 workflow/build/33 browser suite passed; implementation [PR #15 CI passed](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36107616622) | No real export or provider request was used, so MAL XML compatibility beyond the tested subset and live provider response shapes remain unverified. Decision 0001's provider use/storage and public-asset holds remain; M2.7 and M2 exit gate stay open. M3.3 is next |
| 2026-09-25 | M3.3 | V1 pair mean had support but no semantic decision; it could be mistaken for correlation or similarity | Complete decision for future recommendation neighborhoods: support-shrunk adjusted cosine over co-rated user-centered scores, with v1 pair-preference weights explicitly retained until versioned migration | [Decision 0005](decisions/0005-graph-edge-semantics.md) and a hand-computed invented fixture compare four pairs: aligned, co-disliked, opposed, and one-overlap. The synthetic benchmark passes at `1e-12`; full local fixture/typecheck/38 pipeline + 37 web unit/7 ML + 5 workflow/build/33 browser suite passed | The benchmark demonstrates semantic distinctions, not offline accuracy or a tuned shrinkage parameter. M3.4 sign behavior, M3.5–M3.6 bounded candidate work, M3.7 versioning, and M5 split-first evaluation remain open; provider rights hold and M3 exit gate remain unchanged. M3.4 is next |
| 2026-09-25 | M3.4 | Negative v1 pair preference could appear in a graph explanation only when selected after a positive source; it never reduced the score. The Python trainer made every negative edge attractive through `abs(weight)`, and network anime edges used one color | Complete signed/sparse fixture matrix and [decision 0006](decisions/0006-signed-graph-evidence.md); v1 network sign is visible and correctly labeled, nonpositive v1 edges do not enter recommendations or attractive trainer constraints | Hand-computed opposite, co-disliked, neutral, constant/flat, one-overlap, and missing cases pass; high/low rater offsets center identically. Compact/legacy browser and Python loader regressions and the demo network sign test pass. Full local fixture/typecheck/40 pipeline + 37 web unit/8 ML + 5 workflow/build/34 browser suite passed | This changes only current v1 sign consumption, not graph weight meaning, exports, or existing trained models. Future negative similarity and negative user preferences remain gated on M3.7/M4.1 and M5 evaluation. M3.5–M3.7, provider rights, and the M3 exit gate remain open; M3.5 is next |
| 2026-09-25 | M3.5 | The output-edge guard retained first-seen keys and censored their later support/means, while still visiting all pair observations | Complete exact v1 pair aggregation within separate pair-visit and candidate-key budgets; support/magnitude/numeric-ID ordering, optional minimum support and per-anime degree, fail-closed input exhaustion, and CLI selection counts | [Decision 0007](decisions/0007-bounded-pair-selection.md) compares first-encounter, unbounded top-K, approximation, and exact bounded strategies. An invented 8-visit/6-key fixture makes cap 1 choose the later support-3 pair; shuffled rows, support and neighbor limits, numeric ties, malformed inputs, both export formats, and no partial files on budget failure pass. A selected-pair browser fixture exposed and verified a default-threshold rounding fix. Full local fixture/typecheck/47 pipeline + 38 web unit/8 ML + 5 workflow/build/35 browser suite passed | Defaults are guard values, not production capacity or quality estimates. The legacy first-N per-user cap remains order-sensitive and its omissions are reported; M3.6 must replace/measure it, and M3.7 must version selection/truncation metadata. Provider rights and M3 exit gate remain open; M3.6 is next |
| 2026-09-25 | M3.6 | The per-user graph cap selected the first N ratings, so input order affected both pair statistics and user-anime graph edges; skipped observations and resource/coverage effects were unmeasured | Complete seeded SHA-256 bottom-k policy shared by both graph edge types, plus a build-report sidecar and synthetic exact-versus-capped benchmark | [Decision 0008](decisions/0008-seeded-per-user-selection.md) fixes hash input, seed, selection and measurement scopes. Shuffled input and different-seed regressions, compact/legacy CLI and browser compatibility, invalid seed and no-partial-report checks pass. A 250-user invented benchmark retains 6,000/12,000 ratings, 69,000/282,000 pair visits, 180/180 anime, and 8,105/8,460 pair keys; one local run measured 36.405 ms / 84,115,456 bytes exact and 43.724 ms / 84,660,224 bytes capped. Full local fixture/typecheck/50 pipeline + 38 web unit/8 ML + 5 workflow/build/35 browser suite passed | The sidecar is not bound to v1 graph identity, and a normal capped build cannot know true pair-key recall; M3.7 must version/configure artifacts and separate visualization and recommendation sets. The small synthetic run establishes neither speed/memory improvement nor ranking quality. M5 and provider rights holds remain; M3 exit gate stays open and M3.7 is next |
| 2026-09-25 | M3.7 | V1 graph exports lacked in-artifact provenance, and the demo and legacy-only sync paths could reuse recommendation edges as the explorer graph | Complete [decision 0009](decisions/0009-graph-v2-contract.md): compact/legacy v2 recommendation exports share dataset and graph identity, required support, named semantics, settings, and truncation; sync produces a linked compact visualization sample from either output path | Synthetic CLI and temporary-directory sync tests pass for both formats; browser and trainer reject wrong roles, malformed v2, mismatched identities and mixed versions while reading v1. The full local fixture/typecheck/52 pipeline + 41 web unit/10 ML + 5 workflow/build/35 browser suite passed. Existing hand-computed/shuffled/budget and NaN/reference checks satisfy the M3 local exit gate | Existing pair preference and positive-only use remain, with no new adjusted-cosine serving or model retraining. Hashes identify generated content but readers do not recompute them; M5 split-first ranking evaluation and provider/publication holds remain. M4.1 is the next dependency-ready task |
| 2026-09-25 | M4.1 | Every positive imported score became positive watch weight; merely added titles seeded recommendations, and seasonal quickstart silently entered unseen titles as watched | Complete explicit Seen/Liked/Disliked preference with separate importance and confidence; native-scale score thresholds, signed model evidence, like-only positive-pair graph seeds, and v5 state/profile migration with untouched v1/v4 sources and raw backups | Synthetic mapping/migration/ranking/unit and browser journeys passed; full fixture/typecheck/52 pipeline + 46 web unit/10 ML + 5 workflow/build/38 browser suite passed. Low, neutral, unscored, planned, manual override, missing IDs, quota recovery, and seasonal no-mutation cases are covered | M4.4's seasonal non-mutation condition was pulled into M4.1 because the old quickstart contradicted explicit seen semantics; its popularity/content cold start remains unchecked. Existing v1 pair weights and model artifacts remain unchanged. M4.2 owns centralized eligibility; M4.3–M4.8 and the M4 exit gate remain open. No live provider use, production regeneration, or quality lift is claimed. |
| 2026-09-25 | M4.2–M4.3 | Include-only was labeled force-include; eligibility and metadata filtering occurred after hybrid normalization; model failure stopped results, and partial model mapping was silent | Complete one eligibility policy across graph, model, hybrid, graph fallback, and catalog coverage fallback. Include Only narrows candidates already in the active ranking and cannot insert titles into graph or model rankings; watched/excluded titles and absent catalog entries are blocked first. Required genre/year/score filters reject missing metadata. Missing, invalid, or unusably mapped model data uses eligible graph results or a labeled catalog coverage baseline while keeping the requested mode | Invented unit and browser cases cover exclusion precedence, history and preferences, catalog membership, pre-blend filtering, all ranking paths, and absent/corrupt/incompatible/partial model artifacts. Full fixture/typecheck/52 pipeline + 48 web unit/10 ML + 5 workflow/build/47 browser gate passed (see `docs/PROGRESS.md`) | M4.3 was pulled into the same slice because M4.2 requires a real fallback path. Catalog coverage counts positive graph connections; it is a transparent failure baseline, not M4.4's quality/popularity cold start or an evaluated ranking. Metadata prefetch is still bounded and can miss lower candidates (M6.4). M4.4–M4.8 and the M4 exit gate remain open; no provider use, regeneration, or ranking lift is claimed. |
| 2026-09-25 | M4.4 | Pending cold-start path; the existing positive-connection catalog baseline only covered model failure | Complete explicit no-preference sampled-rating-count and known community-score views, genre filtering, and exact shared-genre content baseline for Liked seeds, including automatic sparse-history fallback when metadata supports it. Keep seasonal browsing non-mutating | Invented compact/legacy count and missing-score/genre unit cases, demo and mocked normal-mode browser journeys, cancellation regression, and full local fixture/typecheck/52 pipeline + 50 web unit/10 ML + 5 workflow/build/53 browser gate passed (see `docs/PROGRESS.md`) | Counts are a proxy from the recommendation graph's retained rating edges, not global popularity; community quality covers only known scores. Normal-mode metadata checks are bounded and may miss lower candidates (M6.4). The M3 and M4 status table was reconciled with already completed work. M4.5–M4.8 and the M4 exit gate remain open; no provider calls, production regeneration, or measured ranking lift is claimed. |
| 2026-09-25 | M4.5 | Browser averaged signed item vectors, while trainer metrics evaluated stored user factors; no valid browser-path comparison existed | Complete predeclared, reproducible invented holdout and ridge fold-in comparison in [decision 0010](decisions/0010-user-vector-evaluation.md). Retain the browser average because both methods tied on Hit@3 at one and three observed ratings; the average passed its local quality/latency gate and fold-in missed the promotion threshold | 29 one-rating and 7 three-rating cases; average Hit@3 20/29 and 5/7; fold-in the same. Hand-computed ridge/bias tests, browser model journey, evaluation typecheck, and full local fixture/typecheck/52 pipeline + 52 web unit/10 ML + 5 workflow/build/54 browser gate passed (see `docs/PROGRESS.md`) | This is a fixture-only serving-path decision; the eight item vectors were authored, and cases overlap within users. M5 split-first, leakage-checked evaluation on permitted inputs is still needed for ranking-quality claims and any later method promotion. M4.6–M4.8 and the M4 exit gate remain open. |
| 2026-09-25 | M4.6 | Eligible min–max hybrid lists discarded their minimum scored item, including at a 0%/100% endpoint; component scales and missing-source behavior were not reliable | Complete a protocol-locked synthetic leave-one-user-out comparison in [decision 0011](decisions/0011-hybrid-blend-validation.md); replace min–max with weighted reciprocal-rank fusion after its quality/latency gate passed. Keep full endpoint lists, give an available sole component full effective weight, share ranks for equal scores, retain negative raw scores by relative order, and use anime ID for ties. Label outputs as relative rank points | 29 one-rating and 7 three-rating cases: Hit@3 tied at 20/29 and 5/7; fusion MRR improved from 0.449 to 0.481 and 0.560 to 0.588. Deployed fusion p95 was below 1 ms on the declared 1,000-item host workload. Target-user graph leakage perturbation, reference/runtime parity, edge-case units, browser endpoint and missing-component journeys, and full local fixture/typecheck/52 pipeline + 57 web unit/10 ML + 5 workflow/build/55 browser gate passed (see `docs/PROGRESS.md`) | The model vectors are authored and cases overlap; the fixture result establishes local behavior only, not production quality or calibration. M5 still requires permitted split-first evaluation. M4.7–M4.8 and the M4 exit gate remain open; no provider data, model regeneration, or production deployment was used. |
| 2026-09-25 | M4.7 | Explanations showed only a few unnormalized contributions, omitted model prior/bias and exact hybrid rank terms, and counted graph edges as support without a distinct-title count. Repeated legacy graph edges could be overwritten in the explanation while still affecting the score. | Complete [decision 0012](decisions/0012-score-explanations.md): retain every graph edge contribution, group by actual source title, show distinct counts, model denominator/global mean/item bias/signed terms and mapping coverage, and decompose filtered hybrid rank points with source equations and effective weights. Reconcile displayed sums with a separate rounding adjustment; label unsupported attribution qualitative and state uncertainty. | Hand-computed duplicate-edge, model, partial-map, fusion, rounding, and qualitative tests passed; demo/mock browser tests kept rankings and scores while checking title rendering and escaping. Full local fixture/typecheck/52 pipeline + 62 web unit/10 ML + 5 workflow/build/55 browser gate and both earlier synthetic ranking evaluations passed; [fresh PR fixture CI](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36132192361) passed on implementation commit `835891f` (see `docs/PROGRESS.md`). | The ranking formulas and graph/model artifacts did not change. M4.8 and the M4 exit gate remain open; no confidence calibration or production ranking-quality claim is made. M5 and provider-use holds remain. |
| 2026-09-25 | M4.8 / M4 exit | Eligible ranking previously had no franchise or prerequisite handling; a related side story could fill the top list, and missing relation metadata was not labelled | Complete [decision 0013](decisions/0013-franchise-diversity.md): final-list known-prequel withholding and one-per-known/title-suggested series by default, persistent Allow related titles override, explicit unknown-prerequisite notes. Applied after central eligibility across ranking, fallback, and discovery without changing scores or provider-request budgets. | Predeclared invented top-three distinct families rose from 2 to 3 with NDCG@3 loss 0.095023 (limit 0.10). Missing/empty/malformed/asymmetric/transitive links, repeated/false title cues, off-graph seen history, persistence, escaping, and mocked normal metadata pass. Full local fixture/typecheck/52 pipeline + 70 web unit/10 ML + 5 workflow/build/57 browser gate passed; M4 exit conditions have local evidence in prior M4.1–M4.7 gates. | The authored gains do not measure real-world relevance; relationship metadata may be incomplete and title grouping may err. M5 needs a permitted split-first evaluation and M2.7 retains the provider-use hold. M4 is closed locally, not a web release approval. |
| 2026-09-25 | M5.1 | Existing MF and LightGCN evaluation split centered ratings into train/test after loading a graph built from the full snapshot; no immutable validation IDs or raw-score-first manifest existed | Complete [decision 0014](decisions/0014-raw-interaction-splits.md): strict legacy/compact/raw loaders use raw scores, deduplicate before allocation, pin three-way hashed membership with content/identity digests, and expose validated raw partitions. Seeded per-user holdout is the default; verified event-time splits require explicit trustworthy event-time evidence. | Pinned 13-interaction invented fixture gives 7/3/3 IDs after one duplicate. Six focused Python tests cover row-order and score independence, raw-versus-normalized fields, conflicts, sparse users, temporal ordering/ties, manifest tampering, CLI overwrite/public-output guards. Full local fixture/typecheck/52 pipeline + 70 web unit/16 ML + 5 workflow/build/57 browser gate and CI workflow lint passed. | No existing model evaluator consumes these partitions yet; M5.2–M5.3 must rebuild train-only graph/statistics and prove no held-out leakage. Current provider metadata does not supply verified rating/viewing time; no real-data quality or temporal claim is made. M5 exit gate and source-use hold remain open. |
| 2026-09-25 | M5.2 | The raw split existed but the MF path still loaded centered full-snapshot ratings and a prebuilt full graph; metadata and rating-derived statistics had no isolation boundary | Complete [decision 0015](decisions/0015-train-only-preprocessing.md): a split-first MF route validates the manifest, fits user centering and popularity from only train rows, passes only centered train rows to the existing TypeScript pair core, and uses a separate fixed ID/title metadata snapshot. It fits model parameters with no holdout metric. | Pinned 7-row training fixture yields exact baselines, popularity and 4 pair weights/support; a separate two-user case verifies support 2. Six focused tests cover exact fit hash, fixed metadata, stale manifest, held-out score independence, pair-budget failure, and deterministic model fitting. Full local fixture/typecheck/52 pipeline + 70 web unit/22 ML + 5 workflow/build/57 browser gate passed; see `docs/PROGRESS.md`. | This completes train-only preprocessing on the new route, not the legacy MF/Optuna/LightGCN commands, which remain explicitly invalid for M5 metrics. M5.3 must prove graph/model/training-hash invariance under both score and hidden-interaction perturbations. Metadata source/use and timestamp cutoff need review for any real snapshot. No metric, quality, or release claim; M5 exit and provider-use holds remain open. |
| 2026-09-25 | M5.3 | M5.2's train-only fit had a held-out-score assertion for fit SHA but no model-parameter, full graph-statistic, or hidden-interaction regression | Complete [decision 0016](decisions/0016-fixed-split-leakage-regression.md): perturb validation and test scores and replace separate hidden interaction IDs while preserving exact training IDs/rows; refresh and validate manifests, compare all train statistics/arrays and every MF parameter, and keep file-backed CLI hashes equal. | Five new Python tests pass, including a changed-train-score positive control and fail-closed added-interaction case. Full local pinned fixture/typecheck/52 pipeline + 70 web unit/27 ML + 5 workflow/build/57 browser suite passed; one unrelated page-input timeout in the first browser run passed alone and on full rerun. See `docs/PROGRESS.md`. | A newly added row changes derived train membership and is not a fixed-split invariance case. This proves the invented split-first route only; legacy model scripts remain invalid M5 evidence. M5.4 must establish validation-only selection and one final test report before metrics can count. M5 exit and provider-use holds remain open. |
