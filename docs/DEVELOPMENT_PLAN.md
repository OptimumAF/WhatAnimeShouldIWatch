...
| M3 | In progress | M3.1–M3.6 verified on synthetic inputs. [Decision 0005](decisions/0005-graph-edge-semantics.md) names the v1 pair-preference statistic and chooses support-shrunk adjusted cosine for a future version. [Decision 0006](decisions/0006-signed-graph-evidence.md) separates visualization, candidate, and trainer sign policy. [Decision 0007](decisions/0007-bounded-pair-selection.md) replaces first-encounter v1 output caps with exact selection inside independent input budgets. [Decision 0008](decisions/0008-seeded-per-user-selection.md) makes a per-user cap reproducible and reports synthetic coverage and resource use. M3.7 and the milestone exit gate remain open. |
| M2 | In progress | M2.1–M2.6, M2.8, and M2.9 verified locally; M2.9 fixture PR [CI passed](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36091467488), M2.4's [fresh PR CI passed](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36100629660), and M2.5's [fresh PR CI passed](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36102144479). M2.6 adds transactional schema versions and restricted row/run provenance; [fresh PR CI passed](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36104137643) on implementation commit d050981. [Decision 0001](decisions/0001-provider-data-permissions.md) records provider holds; the [M2.2 audit](audits/public-artifacts-2026-09-24.md) confirms public per-user ratings and graph edges. [Decision 0002](decisions/0002-provider-workflow-gates.md) gates future jobs once merged. [Decision 0003](decisions/0003-provider-request-policy.md) records M2.4's rate policy; [decision 0004](decisions/0004-collection-provenance.md) records M2.6's timestamp and migration rules. M2.7 remains held for permitted source content. M2.8 local and mocked history, native-scale, preview, and file-parser checks passed; [PR #15 fresh CI passed](https://github.com/OptimumAF/WhatAnimeShouldIWatch/actions/runs/36107616622) on implementation commit `6f79d0c`. Real provider export compatibility was not tested. M3.7 is the next fixture-only slice. The exit gate remains open. |
