...
In **Import & Profiles**, choose a local `.txt` file up to 128 KiB, a decompressed MAL-style `.xml` anime list up to 2 MiB, or paste text. Text lines use `animeId[, score[, status[, episodes]]]` or a title in place of the ID (the synthetic demo accepts `101, 9, Completed, 12`). The XML reader accepts anime IDs, titles, `my_status`, `my_watched_episodes`, and `my_score`; it rejects DTDs and malformed fields. Files are read only in the browser. Preview counts, including unscored and unmapped entries, then choose **Merge** or **Replace** and apply. Replace also resets the current watched picks; the preview counts those removals. Re-importing the same file merges by source identity. Imported history, status, episode progress, score and its input scale, and unmatched titles persist with the local recommendation state and named profiles. Unscored seen items are excluded from suggestions without becoming positive ranking evidence. This implementation was verified with synthetic XML and text, not a real account export; no Crunchyroll-native export parser is claimed.

Username import is optional. The entered username goes directly to the selected AniList or MAL provider to read a public list into the same preview. Nothing is applied until the user chooses merge or replace. If direct MAL access fails, the app reports the failure and keeps the current history; it does not try a proxy. A provider field unavailable in a response stays unknown rather than being guessed. AniList history retains the user's declared native score format; mapped scores are converted only when creating provisional watched weights. Sad and neutral three-point smileys stay in history without a positive watched pick. M4.1 still owns the full preference mapping. The M2.1 permissions review still holds provider-dependent product use; fixture tests are not approval for live import, storage, or deployment.
